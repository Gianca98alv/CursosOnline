...
public class EstudianteDAO {
    ...
    Estudiante getById(Integer id_estudiante) throws Exception {
        try {
            String sql = "SELECT * FROM Estudiante WHERE id_estudiante = %d";
            sql = String.format(sql, id_estudiante);
            ResultSet rs = db.executeQuery(sql);
            if (rs.next()) {
                return map(rs);
            }
            throw new SQLException("/estudiante/?=" + id_estudiante + " Does not exist in DataBase");
        } catch (Exception e) {
